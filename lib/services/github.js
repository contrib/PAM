...
function Repo(repo) {
  ...
  this.findCollaboratorsTeam();
}

Repo.prototype.setCollaborators = function(id) {
  var self = this;

  client.orgs.getTeamMembers({ id: id }, function(err, members) {
    if (err) { return new Error('Could not retrieve team members'); }
    members = _.reject(members, { login: 'pam' });
    self.collaborators = _.union(self.collaborators, members);
  });
};

Repo.prototype.findCollaboratorsTeam = function() {
  ...
  client.orgs.getTeams({
    ...
    if (err) { return new Error('Unable to retrieve teams for this organization'); }
    var team = _.reject(teams, { name: 'Owners' })[0];
    self.setCollaborators(team.id);
  });
};
